...
$jenkinsFolderViews = "your folder for view"
...
$sh = "wget -q --no-check-certificate --auth-no-challenge --user $($user) --password $($pass) '$($jenkinsUrl)/job/$($jenkinsFolderViews)/view/All/api/json?tree=jobs[name,color]' -O $($jobsJsonOutput) > /dev/null"
...
if ($jobRunning.count -gt 0 -and $jobRunning -notmatch $LastRunningJob) {
	$shBuildN = "wget -q --no-check-certificate --auth-no-challenge --user $($user) --password $($pass) '$($jenkinsUrl)/job/$($jenkinsFolderViews)/job/$($jobRunning)/api/json?tree=builds[number]' -O $($buildNumberJsonOutput) > /dev/null"
	bash -c $shBuildN
	...
	$BlogButton = New-BTButton -Content 'View Log' -Arguments "$($jenkinsUrl)/job/$($jenkinsFolderViews)/job/$($jobRunning)/$($lastBuildNumber)/console"
